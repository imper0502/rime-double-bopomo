...
### 簡拼規則【本功能預設關閉】
移除方案檔（schema）中註解後，正常輸入時，若遇到雙拼空碼、奇數輸入鍵時，就會有簡拼。（Rime設計理念：假設使用者所有輸入的按鍵都是正確的。）
以下提供**一定**被視為簡拼的方法：
  1. 注音聲母（或大寫一碼韻母） + 聲調（數字１～５）；所以改用６～０選字（預設一頁五字）【本功能預設關閉】  
     **e.g.** ➡️ `o1r4k4l4`➡️ `ㄕˉㄖˋㄎˋㄌˋ`➡️ 生日快樂
  2. 注音聲母（或大寫一碼韻母） + 分詞鍵【'】（【'】在分號鍵的右邊。）【本功能預設關閉】  
     **e.g.** ➡️ `o'r'k'l'`➡️ `ㄕ'ㄖ'ㄎ'ㄌ'`➡️ 生日快樂
  3. 按下【;】後，強制使用簡拼。【本功能預設關閉】  
...
### 檔名解釋
  ...
  - `full_bopomofo.schema.yaml`：全碼注音，給初學者的過度方案。
